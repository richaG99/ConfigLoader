...
var LoggableError = require('node-error').LoggableError;

var logError = function(msg,callback){
    logger.error(msg);
    callback(new LoggableError(msg));
    process.exit();
        ...
        if ( currentline.length > 0 && currentline.indexOf(';') == -1 ) { // leave the comment line or empty line
            ...
            if (lineMembers.length == 1) { //group case
                ...
                if (groupString != null && groupString.length > 0 && groupString.charAt(0) == enclosures.groupOpening && groupString.charAt(groupString.length - 1) == enclosures.groupClosing)
                    ...
                    logError( errors.GROUPS_ARE_NOT_PROPERLY_FORMED +"," + constants.LINE_NUMBER+ ": " + lineNumber + " , "+ constants.INCORRECT_LINE + " : " + line , callback );
            ...
            } else if (lineMembers.length == 2) //property case
            {
                ...
                {
                    logError(errors.AT_LEAST_ONE_GROUP_SHOULD_BE_DEFINED +"," + constants.LINE_NUMBER +" : " + lineNumber + ","+ constants.INCORRECT_LINE + " : " + line, callback );
                }
                ...
                if (settingLabel != null && settingLabel.length > 0) {
                    ...
                    if (overRideSettings.length > 2) { // error case like path<produc<tion>> or path<production><abc> they are not valid
                        logError(errors.MORE_THAN_ONE_OVERRIDE_OPENINGS_NOT_ALLOWED+ ","+ constants.LINE_NUMBER + ": " + lineNumber + " ,"+ constants.INCORRECT_LINE +": " + line ,callback);
                ...
                logError(errors.COFIG_FILE_NOT_PROPERTLY_FORMED + " ," + constants.LINE_NUMBER +": " + lineNumber + " ,"+ constants.INCORRECT_LINE +": " + line , callback);
            }
            lineNumber++;
        }
        if (last) {
